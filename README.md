...
- [Grounding with Bing Search](https://learn.microsoft.com/azure/ai-services/openai/how-to/search?tabs=bing-grounding) — 使用 `azure_web_search` 工具需要在 Azure AI Foundry 中連接 Grounding with Bing Search 資源（選填，不設定仍可使用 `web_search` 免費搜尋）
...
```bash
dotnet run --project AgentCraftLab.Autonomous.Playground
```
